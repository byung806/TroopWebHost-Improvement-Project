...
def get_logged_in_session(username, password):
    with requests.Session() as session:
        ...
        session.cookies.set("Application_ID", config["TROOP_ID"])
...
def get_data(logged_in_session):
    ADULT_TRAINING_URL = f'https://www.troopwebhost.org/FormList.aspx?Menu_Item_ID={config["ADULT_TRAINING_URL_ID"]}&Stack=0'
    SEND_EMAIL_URL = 'https://www.troopwebhost.org/FormDetail.aspx?Menu_Item_ID={config["SEND_EMAIL_URL_ID]}&Stack=1'
    ...
    for entry in soup.findAll("table"):
        ...
        for tr in entry.findAll("tr", id=True):
            ...
            column = columns[
                column_td.text.strip()
            ]  # could cause error if not in columns
        ...
        for label, value in pairs:
            ...
            if label == "Email":
                ...
                if "@" in possible_emails[1]:
                    value = [
                        possible_emails[0][:-4].strip(),
                        possible_emails[1][:-5].strip(),
                    ]
                else:
    ...
    adult_training_data = {
        (p[0], p[3], p[6], *email_data[p[0]])
        for p in adult_training_data
        if len(email_data[p[0]]) == 1
    }
